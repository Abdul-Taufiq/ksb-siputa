...
namespace App\Http\Controllers\Siadit;

use App\Http\Controllers\Controller;
use App\Models\{LogActivity, User};
use App\Models\Siadit\USiadit;
...
class USiaditController extends Controller
{
    ...
    private $subjek = 'Pengajuan User (SiAdit)';
    private $subjek_status = 'Status | Pengajuan User (SiAdit)';
    ...
    public function index(Request $request)
    {
        ...
        if (request()->ajax()) {
            ...
            return DataTables()->of($data)
                ...
                ->addColumn('action', function ($data) {
                    $button = '<a data-toggle="modal" data-target="#myModal' . $data->id_siadit_user . '" id="' . $data->id_siadit_user . '"
                    ...
                    switch (auth()->user()->jabatan) {
                        ...
                        case 'Kasi Komersial':
                            if ($data->status_pincab == "Approve" || $data->status_pincab == "Reject") {
                                ...
                                $button .= '<a data-toggle="modal" data-target="#modalEdit' . $data->id_siadit_user . '" id="' . $data->id_siadit_user . '"
                        ...
                        case 'Staf Area':
                            if ($data->status_sdm != null) {
                                ...
                                $button .= '<a href="/user-email-pengajuan/' . $data->id_siadit_user . '/edit" data-toggle="tooltip" 
        ...
        return view('Page-siadit.siadit-u.index', ['title' => 'Pengajuan User Si-Adit']);
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($cek == 0) {
            $urut = 0001;
            $nomer = $cabang . '/SiADIT-P/' . $thn . '/0001';
        } else {
            $ambil = USiadit::all()->last();
            ...
            if ($cekTahun != $thn) {
                ...
                $nomer = $cabang . '/SiADIT-P/' . $thn . '/' . $urut;
            }
        }
        ...
        if ($request->kategori == 'Yes') {
            $data->aktif = $request->aktif;
            $data->non_aktif = $request->non_aktif;
        } elseif ($request->kategori == 'No') {
            $data->aktif = $request->aktif;
            $data->non_aktif = null;
        }
        $data->save();
        ...
        return redirect('user-siadit')->with('AlertSuccess', "Pengajuan User Berhasil Dikirim!");
    }



    public function show(USiadit $uSiadit)
    {
        return view('Page-siadit.siadit-u.show', compact('uSiadit'));
    }



    public function edit(USiadit $uSiadit)
    {
    ...
    public function update(Request $request, USiadit $uSiadit)
    {
        $uSiadit->keperluan = $request->keperluan;
        $uSiadit->nik = $request->nik;
        $uSiadit->nama = $request->nama;
        $uSiadit->jabatan = $request->jabatan;
        $uSiadit->no_telp = $request->no_telp;
        $uSiadit->keterangan = $request->keterangan;
        if ($request->kategori == 'Yes') {
            $uSiadit->aktif = $request->aktif;
            $uSiadit->non_aktif = $request->non_aktif;
        } elseif ($request->kategori == 'No') {
            $uSiadit->aktif = $request->aktif;
            $uSiadit->non_aktif = null;
        }
        $uSiadit->updated_at = now();
        $uSiadit->save();


        // Log Activity
        $LogAksi = '(u) Pengajuan User SiAdit';
        $this->LogActivity($uSiadit, $LogAksi);

        return redirect('user-siadit')->with('AlertSuccess', "Pengajuan User Berhasil DiUpdate!");
    }



    public function destroy(USiadit $uSiadit)
    {
        //
    }
    ...
    public function ResponApprove(Request $request, $idEncrypt)
    {
        ...
        $data = USiadit::where('id_siadit_user', $ids)->first();
        $jabatan = auth()->user()->jabatan;
        $nama = auth()->user()->nama;
        ...
        // Log Activity
        $LogAksi = '(cs) Approve Pengajuan User Siadit';
        $this->LogActivity($data, $LogAksi);

        return redirect('user-siadit')->with('AlertSuccess', "Pengajuan Berhasil Dilakukan Perubahan Status!");
    }
    ...
    public function ResponReject(Request $request, $idEncrypt)
    {
        ...
        // Log Activity
        $LogAksi = '(cs) Rejected Pengajuan User Siadit';
        $this->LogActivity($data, $LogAksi);
    ...
    private function statusNothing($data)
    {
        ...
        $status .= '<a class="dropdown-item approve" href="#" data-toggle="modal" data-target="#modalApprove" data-id="' . encrypt($data->id_siadit_user) . '" data-kode_form="' . $data->kode_form . '">Approve</a>';
        $status .= '<a class="dropdown-item reject" href="#" data-toggle="modal" data-target="#modalReject" data-id="' . encrypt($data->id_siadit_user) . '" data-kode_form="' . $data->kode_form . '">Reject</a>';
        $status .= '</div>';
        $status .= '</div>';
        return $status;
    }
