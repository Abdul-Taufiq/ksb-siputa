...
class PSiaditController extends Controller
{
    public function index(Request $request)
    {
        ...
        if (request()->ajax()) {
            ...
            return DataTables()->of($data)
                ...
                ->addColumn('action', function ($data) {
                    $button = '<a data-toggle="modal" data-target="#myModal' . $data->id_siadit_perubahan . '" id="' . $data->id_siadit_perubahan . '"
                    ...
                    switch (auth()->user()->jabatan) {
                        ...
                        case 'Kasi Komersial':
                            if ($data->status_pincab == "Approve" || $data->status_pincab == "Reject") {
                                ...
                                $button .= '<a data-toggle="modal" data-target="#modalEdit' . $data->id_siadit_perubahan . '" id="' . $data->id_siadit_perubahan . '"
        ...
        return view('Page-siadit.siadit-per.index', ['title' => 'Perubahan/Penghapusan Data SPK (SiAdit)']);
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($cek == 0) {
            $urut = 0001;
            $nomer = $cabang . '/SiADIT-C/' . $thn . '/0001';
        } else {
            $ambil = PSiadit::all()->last();
            ...
            if ($cekTahun != $thn) {
                ...
                $nomer = $cabang . '/SiADIT-C/' . $thn . '/' . $urut;
            }
        }
        ...
        // Log Activity
        $LogAksi = '(+) Pengajuan Perubahan SiAdit';
    ...
    public function show(PSiadit $pSiadit)
    {
        return view('Page-siadit.siadit-per.show', compact('pSiadit'));
    }



    public function edit(PSiadit $pSiadit)
    {
    ...
    public function update(Request $request, PSiadit $pSiadit)
    {
        $pSiadit->keperluan = $request->keperluan_edit;
        $pSiadit->no_spk = $request->no_spk_edit;
        $pSiadit->nama_nasabah = $request->nama_nasabah_edit;
        $pSiadit->data_salah = $request->data_salah_edit;
        $pSiadit->pembetulan = $request->pembetulan_edit;
        $pSiadit->user = $request->user_edit;
        $pSiadit->keterangan = $request->keterangan_edit;
        $pSiadit->updated_at = now();
        $pSiadit->save();

        // Log Activity
        $LogAksi = '(u) Update Perubahan SiAdit';
        $this->LogActivity($pSiadit, $LogAksi);

        return redirect('siadit-perubahan')->with('AlertSuccess', "Pengajuan Perubahan Data Berhasil Diupdate!");
    }



    public function destroy(PSiadit $pSiadit)
    {
        //
    }
    ...
    public function ResponApprove(Request $request, $idEncrypt)
    {
        ...
        $data = PSiadit::where('id_siadit_perubahan', $ids)->first();
        $jabatan = auth()->user()->jabatan;
        $nama = auth()->user()->nama;
        ...
        // Log Activity
        $LogAksi = '(cs) Approve Pengajuan Perubahan Data (SIADIT)';
        $this->LogActivity($data, $LogAksi);

        return redirect('siadit-perubahan')->with('AlertSuccess', "Pengajuan Berhasil Dilakukan Perubahan Status!");
    }
    ...
    public function ResponReject(Request $request, $idEncrypt)
    {
        ...
        // Log Activity
        $LogAksi = '(cs) Rejected Pengajuan Perubahan Data (SIADIT)';
        $this->LogActivity($data, $LogAksi);
    ...
    private function statusNothing($data)
    {
        ...
        $status .= '<a class="dropdown-item approve" href="#" data-toggle="modal" data-target="#modalApprove" data-id="' . encrypt($data->id_siadit_perubahan) . '" data-kode_form="' . $data->kode_form . '">Approve</a>';
        $status .= '<a class="dropdown-item reject" href="#" data-toggle="modal" data-target="#modalReject" data-id="' . encrypt($data->id_siadit_perubahan) . '" data-kode_form="' . $data->kode_form . '">Reject</a>';
        $status .= '</div>';
        $status .= '</div>';
        return $status;
    }
    ...
    private function SendEmail($data, $userPenerima, $url, $title, $message)
    {
        Mail::send('email.notif.notif-pengajuan',  [
            ...
            $message->subject('Pengajuan Perubahan Data (SIADIT)');
        });
