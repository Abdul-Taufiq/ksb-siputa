...
use App\Models\Pefindo\Pefindo;
use App\Models\User;
...
class PefindoController extends Controller
{
    ...
    private $subjek = 'Pengajuan User WebSakep';
    private $subjek_status = 'Status | Pengajuan User WebSakep';
        ...
        if (request()->ajax()) {
            ...
            switch ($jabatan) {
                ...
                case 'Analis Area':
                    if (!empty($request->kode)) {
                        ...
                        if (!empty($request->min)) {
                            $data = Pefindo::where('id_cabang', $id_cabang)
                ...
                case 'Internal Audit':
                    if (!empty($request->kode)) {
                        ...
                        if (!empty($request->min)) {
                            $data = Pefindo::whereIn('status_pincab', ['Approve', '--'])
                                ->whereBetween('created_at', [$awal, $akhir])
                ...
                case 'TSI':
                    if (!empty($request->kode)) {
                        ...
                        if (!empty($request->min)) {
                            ...
                            $data = Pefindo::where('status_dirops', 'Approve')->orderBy('created_at', 'desc')->get();
                        }
                    }
                    break;
                default:
            ...
            return DataTables()->of($data)
                ...
                ->addColumn('action', function ($data) {
                    $button = '<a data-toggle="modal" data-target="#myModal' . $data->id_pefindo . '" id="' . $data->id_pefindo . '"
                    ...
                    switch (auth()->user()->jabatan) {
                        ...
                        case 'Kasi Komersial':
                            if ($data->status_pincab == "Approve" || $data->status_pincab == "Reject") {
                                ...
                                $button .= '<a data-toggle="modal" data-target="#modalEdit' . $data->id_pefindo . '" id="' . $data->id_pefindo . '"
                        ...
                        case 'Analis Area':
                            if ($data->status_sdm != null) {
                                ...
                                $button .= '<a href="/user-email-pengajuan/' . $data->id_pefindo . '/edit" data-toggle="tooltip" 
    ...
    public function show(Pefindo $pefindo)
    {
        return view('Page-pefindo.pefindo-p.show', compact('pefindo'), ['title' => 'Show Data']);
    }


    public function edit(Pefindo $pefindo)
    {
    ...
    public function update(Request $request, Pefindo $pefindo)
    {
        $pefindo->keperluan = $request->keperluan;
        $pefindo->nik = $request->nik;
        $pefindo->nama = $request->nama;
        $pefindo->jabatan = $request->jabatan;
        $pefindo->no_telp = $request->no_telp;
        $pefindo->keterangan = $request->keterangan;
        $pefindo->updated_at = now();
        $pefindo->save();
    ...
    public function destroy(Pefindo $pefindo)
    {
        //
    }
    ...
    public function ResponApprove(Request $request, $idEncrypt)
    {
        ...
        $data = Pefindo::where('id_pefindo', $ids)->first();
        $jabatan = auth()->user()->jabatan;
        $nama = auth()->user()->nama;
        ...
        switch ($jabatan) {
            case 'Pimpinan Cabang':
                ...
                // Send Email Double
                $userPenerima = User::where('jabatan', 'Direktur Operasional')->where('email', 'not like', '%dummy%')->where('email', 'not like', '%alt%')->where('status', 'Aktif')->where('email', 'like', '%@gmail.com')->get();
    ...
    private function statusNothing($data)
    {
        ...
        $status .= '<a class="dropdown-item approve" href="#" data-toggle="modal" data-target="#modalApprove" data-id="' . encrypt($data->id_pefindo) . '" data-kode_form="' . $data->kode_form . '">Approve</a>';
        $status .= '<a class="dropdown-item reject" href="#" data-toggle="modal" data-target="#modalReject" data-id="' . encrypt($data->id_pefindo) . '" data-kode_form="' . $data->kode_form . '">Reject</a>';
        $status .= '</div>';
        $status .= '</div>';
        return $status;
    }
