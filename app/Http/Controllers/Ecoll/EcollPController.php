...
class EcollPController extends Controller
{
    // load services
    ...
    private $subjek = 'Pengajuan User Ecoll';
    private $subjek_status = 'Status | Pengajuan User Ecoll';
    ...
    public function index(Request $request)
    {
        ...
        if (request()->ajax()) {
            ...
            switch ($jabatan) {
                ...
                case 'Analis Area':
                    if (!empty($request->kode)) {
                        ...
                        if (!empty($request->min)) {
                            $data = EcollP::where('id_cabang', $id_cabang)
                ...
                case 'Direktur Utama':
                    if (!empty($request->kode)) {
                        ...
                        if (!empty($request->min)) {
                            $data = EcollP::where('status_sdm', 'Approve')
                                ->whereBetween('created_at', [$awal, $akhir])
                                ->when($reqCabang != 99, fn($query) => $query->where('id_cabang', $reqCabang))
                                ->orderBy('created_at', 'desc')->get();
                        } else {
                            $data = EcollP::where('status_sdm', 'Approve')->orderBy('created_at', 'desc')->get();
                        }
                    }
                    break;
                case 'TSI':
                    if (!empty($request->kode)) {
                        ...
                        if (!empty($request->min)) {
                            $data = EcollP::where('status_dirops', 'Approve')
                                ->whereBetween('created_at', [$awal, $akhir])
                                ->when($reqCabang != 99, fn($query) => $query->where('id_cabang', $reqCabang))
                                ->orderBy('created_at', 'desc')->get();
                        } else {
                            $data = EcollP::where('status_dirops', 'Approve')->orderBy('created_at', 'desc')->get();
                        }
                    }
                    break;
                default:
            ...
            return DataTables()->of($data)
                ...
                ->addColumn('action', function ($data) {
                    $button = '<a data-toggle="modal" data-target="#myModal' . $data->id_ecoll_p . '" id="' . $data->id_ecoll_p . '"
                    ...
                    switch (auth()->user()->jabatan) {
                        ...
                        case 'Kasi Komersial':
                            if ($data->status_pincab == "Approve" || $data->status_pincab == "Reject") {
                                ...
                                $button .= '<a data-toggle="modal" data-target="#modalEdit' . $data->id_ecoll_p . '" id="' . $data->id_ecoll_p . '"
                        ...
                        case 'Analis Area':
                            if ($data->status_sdm != null) {
                                ...
                                $button .= '<a href="/user-email-pengajuan/' . $data->id_ecoll_p . '/edit" data-toggle="tooltip" 
        ...
        return view('Page-ecoll.ecoll-p.index', ['title' => 'Pengajuan User MBS']);
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($cek == 0) {
            $nomer = $cabang . '/ECOLL-P/' . $thn . '/0001';
        } else {
            $ambil = EcollP::all()->last();
        ...
        return redirect('user-ecoll')->with('AlertSuccess', "Pengajuan Berhasil Dikirim!");
    }


    public function show(EcollP $ecollP)
    {
        return view('Page-ecoll.ecoll-p.show', compact('ecollP'), ['title' => 'Show Data']);
    }


    public function edit(EcollP $ecollP)
    {
    ...
    public function update(Request $request, EcollP $ecollP)
    {
        $ecollP->keperluan = $request->keperluan;
        $ecollP->nik = $request->nik;
        $ecollP->nama = $request->nama;
        $ecollP->jabatan = $request->jabatan;
        $ecollP->no_telp = $request->no_telp;
        $ecollP->keterangan = $request->keterangan;
        $ecollP->updated_at = now();
        $ecollP->save();

        // Log Activity
        $LogAksi = '(u) Edit Pengajuan User Ecoll';
        $this->LogActivity($ecollP, $LogAksi);

        return redirect('user-ecoll')->with('AlertSuccess', "Pengajuan Berhasil Diedit!");
    }


    public function destroy(EcollP $ecollP)
    {
        //
    }
    ...
    public function ResponApprove(Request $request, $idEncrypt)
    {
        ...
        $data = EcollP::where('id_ecoll_p', $ids)->first();
        $jabatan = auth()->user()->jabatan;
        $nama = auth()->user()->nama;
        ...
        // Log Activity
        $LogAksi = '(cs) Approve Pengajuan User Ecoll';
        $this->LogActivity($data, $LogAksi);

        return redirect('user-ecoll')->with('AlertSuccess', "Pengajuan Berhasil Dilakukan Perubahan Status!");
    }
    ...
    public function ResponReject(Request $request, $idEncrypt)
    {
        ...
        // Log Activity
        $LogAksi = '(cs) Rejected Pengajuan User Ecoll';
        $this->LogActivity($data, $LogAksi);
    ...
    private function statusNothing($data)
    {
        ...
        $status .= '<a class="dropdown-item approve" href="#" data-toggle="modal" data-target="#modalApprove" data-id="' . encrypt($data->id_ecoll_p) . '" data-kode_form="' . $data->kode_form . '">Approve</a>';
        $status .= '<a class="dropdown-item reject" href="#" data-toggle="modal" data-target="#modalReject" data-id="' . encrypt($data->id_ecoll_p) . '" data-kode_form="' . $data->kode_form . '">Reject</a>';
        $status .= '</div>';
        $status .= '</div>';
        return $status;
    }
