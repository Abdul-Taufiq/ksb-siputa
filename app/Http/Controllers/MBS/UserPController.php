...
class UserPController extends Controller
{
    ...
    private $subjek = 'Pengajuan User (MSO)';
    private $subjek_status = 'Status | Pengajuan User (MSO)';
    ...
    public function index(Request $request)
    {
        ...
        if (request()->ajax()) {
            ...
            switch ($jabatan) {
                ...
                case 'Staf Area':
                    if (!empty($request->kode)) {
                        ...
                        if (!empty($request->min)) {
                            $data = UserP::where('id_cabang', $id_cabang)
                ...
                case 'Direktur Operasional':
                    if (!empty($request->kode)) {
                        ...
                        if (!empty($request->min)) {
                            $data = UserP::where('status_sdm', 'Approve')
                                ->whereBetween('created_at', [$awal, $akhir])
                                ->when($reqCabang != 99, fn($query) => $query->where('id_cabang', $reqCabang))
                                ->orderBy('created_at', 'desc')->get();
                        } else {
                            $data = UserP::where('status_sdm', 'Approve')->orderBy('created_at', 'desc')->get();
                        }
                    }
                    break;
                case 'TSI':
                    if (!empty($request->kode)) {
                        ...
                        if (!empty($request->min)) {
                            $data = UserP::where('status_dirops', 'Approve')
                                ->whereBetween('created_at', [$awal, $akhir])
                                ->when($reqCabang != 99, fn($query) => $query->where('id_cabang', $reqCabang))
                                ->orderBy('created_at', 'desc')->get();
                        } else {
                            $data = UserP::where('status_dirops', 'Approve')->orderBy('created_at', 'desc')->get();
                        }
                    }
                    break;
                default:
            ...
            return DataTables()->of($data)
                ...
                ->addColumn('action', function ($data) {
                    $button = '<a data-toggle="modal" data-target="#myModal' . $data->id_msop . '" id="' . $data->id_msop . '"
                    ...
                    switch (auth()->user()->jabatan) {
                        ...
                        case 'Kasi Komersial':
                            if ($data->status_pincab == "Approve" || $data->status_pincab == "Reject") {
                                ...
                                $button .= '<a data-toggle="modal" data-target="#modalEdit' . $data->id_msop . '" id="' . $data->id_msop . '"
        ...
        return view('Page-mso.mso-p.index', ['title' => 'Pengajuan User MBS']);
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($cek == 0) {
            $urut = 0001;
            $nomer = $cabang . '/MBSUS-P/' . $thn . '/0001';
        } else {
            $ambil = UserP::all()->last();
        ...
        if ($request->keperluan == 'Permohonan User Baru (Alternate)') {
            if ($request->tgl_akhir == null) {
                $data->aktif = $request->tgl_awal;
                $data->non_aktif = null;
            } else {
                $data->aktif = $request->tgl_awal;
                $data->non_aktif = $request->tgl_akhir;
            }
        }
        ...
        return redirect('mso-pengajuan')->with('AlertSuccess', "Pengajuan Berhasil Dikirim!");
    }


    public function show(UserP $userP)
    {
        return view('Page-mso.mso-p.show', compact('userP'));
    }
    ...
    {
        $userP->keperluan = $request->keperluan;
        $userP->nik = $request->nik;
        $userP->nama = $request->nama;
        $userP->jabatan = $request->jabatan;
        $userP->no_telp = $request->no_telp;
        $userP->keterangan = $request->keterangan;
        $userP->updated_at = now();
        $userP->save();

        // Log Activity
        $LogAksi = '(u) Edit Pengajuan User MSO';
        $this->LogActivity($userP, $LogAksi);

        return redirect('mso-pengajuan')->with('AlertSuccess', "Pengajuan Berhasil Di Edit!");
    }
    ...
    public function ResponApprove(Request $request, $idEncrypt)
    {
        ...
        $data = UserP::where('id_msop', $ids)->first();
        $jabatan = auth()->user()->jabatan;
        $nama = auth()->user()->nama;
    ...
    public function ResponReject(Request $request, $idEncrypt)
    {
        ...
        // Log Activity
        $LogAksi = '(cs) Rejected Pengajuan User MSO';
        $this->LogActivity($data, $LogAksi);
    ...
    private function statusNothing($data)
    {
        ...
        $status .= '<a class="dropdown-item approve" href="#" data-toggle="modal" data-target="#modalApprove" data-id="' . encrypt($data->id_msop) . '" data-kode_form="' . $data->kode_form . '">Approve</a>';
        $status .= '<a class="dropdown-item reject" href="#" data-toggle="modal" data-target="#modalReject" data-id="' . encrypt($data->id_msop) . '" data-kode_form="' . $data->kode_form . '">Reject</a>';
        $status .= '</div>';
        $status .= '</div>';
        return $status;
    }
