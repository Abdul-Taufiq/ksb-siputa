...
namespace App\Http\Controllers\Slik;
...
class PSlikController extends Controller
{
    ...
    private $subjek = 'Pengajuan User (SLIK)';
    private $subjek_status = 'Status | Pengajuan User (SLIK)';
    ...
    public function index(Request $request)
    {
        ...
        if (request()->ajax()) {
            ...
            return DataTables()->of($data)
                ...
                ->addColumn('action', function ($data) {
                    $button = '<a data-toggle="modal" data-target="#myModal' . $data->id_slik . '" id="' . $data->id_slik . '"
                    ...
                    switch (auth()->user()->jabatan) {
                        ...
                        case 'Kasi Komersial':
                            if ($data->status_pincab == "Approve" || $data->status_pincab == "Reject") {
                                ...
                                $button .= '<a data-toggle="modal" data-target="#modalEdit' . $data->id_slik . '" id="' . $data->id_slik . '"
                        ...
                        case 'Staf Area':
                            if ($data->status_sdm != null) {
                                ...
                                $button .= '<a href="/user-email-pengajuan/' . $data->id_slik . '/edit" data-toggle="tooltip" 
        ...
        return view('Page-slik.slik-p.index', ['title' => 'Pengajuan User SLIK']);
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($cek == 0) {
            $nomer = $cabang . '/USLIK-P/' . $thn . '/0001';
        } else {
            $ambil = Pslik::all()->last();
        ...
        return redirect('slik-pengajuan')->with('AlertSuccess', "Pengajuan Berhasil Dikirim!");
    }


    public function show(Pslik $pslik)
    {
        return view('Page-slik.slik-p.show', compact('pslik'), ['title' => 'Show Data']);
    }


    public function edit(Pslik $pslik)
    {
    ...
    public function update(Request $request, Pslik $pslik)
    {
        $pslik->keperluan = $request->keperluan;
        $pslik->nik = $request->nik;
        $pslik->nama = $request->nama;
        $pslik->jabatan = $request->jabatan;
        $pslik->no_telp = $request->no_telp;
        $pslik->keterangan = $request->keterangan;
        $pslik->updated_at = now();
        $pslik->save();


        // Log Activity
        $LogAksi = '(u) Update Pengajuan User SLIK';
        $this->LogActivity($pslik, $LogAksi);

        return redirect('slik-pengajuan')->with('AlertSuccess', "Pengajuan Berhasil Di Edit!");
    }


    public function destroy(Pslik $pslik)
    {
        //
    }
    ...
    public function ResponApprove(Request $request, $idEncrypt)
    {
        ...
        $data = Pslik::where('id_slik', $ids)->first();
        $jabatan = auth()->user()->jabatan;
        $nama = auth()->user()->nama;
        ...
        // Log Activity
        $LogAksi = '(cs) Approve Pengajuan User SLIK';
        $this->LogActivity($data, $LogAksi);

        return redirect('slik-pengajuan')->with('AlertSuccess', "Pengajuan Berhasil Dilakukan Perubahan Status!");
    }
    ...
    public function ResponReject(Request $request, $idEncrypt)
    {
        ...
        // Log Activity
        $LogAksi = '(cs) Rejected Pengajuan User SLIK';
        $this->LogActivity($data, $LogAksi);
    ...
    private function statusNothing($data)
    {
        ...
        $status .= '<a class="dropdown-item approve" href="#" data-toggle="modal" data-target="#modalApprove" data-id="' . encrypt($data->id_slik) . '" data-kode_form="' . $data->kode_form . '">Approve</a>';
        $status .= '<a class="dropdown-item reject" href="#" data-toggle="modal" data-target="#modalReject" data-id="' . encrypt($data->id_slik) . '" data-kode_form="' . $data->kode_form . '">Reject</a>';
        $status .= '</div>';
        $status .= '</div>';
        return $status;
    }
