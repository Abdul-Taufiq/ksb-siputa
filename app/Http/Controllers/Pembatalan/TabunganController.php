...
class TabunganController extends Controller
{
    public function index(Request $request)
    {
        ...
        if (request()->ajax()) {
            switch ($jabatan) {
                ...
                case 'Pimpinan Cabang':
                    if (!empty($request->kode)) {
                        $data = Tabungan::where('kode_form', $kode)
                            ->OrderBy('created_at', 'desc')->get();
                    } else {
                        if (!empty($request->min)) {
                            $data = Tabungan::where('id_cabang', $id_cabang)
                ...
                case 'Internal Audit':
                    if (!empty($request->kode)) {
                        ...
                        if (!empty($request->min)) {
                            $data = Tabungan::whereIn('status_pincab', ['Approve', '--'])
                                ->whereBetween('created_at', [$awal, $akhir])
                                ->when($reqCabang != 99, fn($query) => $query->where('id_cabang', $reqCabang))
                                ->get();
                        } elseif (!empty($request->cari)) {
                            $data = Tabungan::where('kode_form', $request->cari)
                                ->orderBy('created_at', 'desc')->get();
                        } else {
                            $data = Tabungan::whereIn('status_pincab', ['Approve', '--'])
                                ->OrderBy('created_at', 'desc')->get();
                ...
                case 'Direktur Operasional':
                    if (!empty($request->kode)) {
                        ...
                        if (!empty($request->min)) {
                            $data = Tabungan::where('status_pembukuan', "SendedToDirops")
                                ->orwhere('status_pembukuan', 'Approve')
                                ->whereBetween('created_at', [$awal, $akhir])
                                ->when($reqCabang != 99, fn($query) => $query->where('id_cabang', $reqCabang))
                                ->orderBy('created_at', 'desc')->get();
                        } else {
                            $data = Tabungan::where('status_pembukuan', 'Approve')
                                ->orwhere('status_pembukuan', 'SendedToDirops')
                                ->orderBy('created_at', 'desc')->get();
                        }
                    }
                    break;
                case 'TSI':
                    if (!empty($request->kode)) {
                        ...
                        if (!empty($request->min)) {
                            $data = Tabungan::where('status_dirops', 'Approve')
                                ->whereBetween('created_at', [$awal, $akhir])
                                ->when($reqCabang != 99, fn($query) => $query->where('id_cabang', $reqCabang))
                                ->orderBy('created_at', 'desc')->get();
                        } else {
                            $data = Tabungan::where('status_dirops', 'Approve')->orderBy('created_at', 'desc')->get();
                        }
                    }
                    break;
                default:
            ...
            return DataTables()->of($data)
                ...
                ->addColumn('action', function ($data) {
                    $button = '<a data-toggle="modal" data-target="#myModal' . $data->id_tabungan . '" id="' . $data->id_tabungan . '"
                    ...
                    switch (auth()->user()->jabatan) {
                        ...
                        case 'Kepala Kantor Kas':
                            if ($data->status_pincab == "Approve" || $data->status_pincab == "Reject") {
                                ...
                                $button .= '<a data-toggle="modal" data-target="#modalEdit' . $data->id_tabungan . '" id="' . $data->id_tabungan . '"
        ...
        return view('Page.Pembatalan.Tabungan.index', ['title' => 'Pembatalan Transaksi Tabungan']);
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($cek == 0) {
            $urut = 0001;
            $nomer = $cabang . '/BTT-TBG/' . $thn . '/0001';
        } else {
            $ambil = Tabungan::all()->last();
        ...
        // Log Activity
        $LogAksi = '(+) Pengajuan Pembatalan Transaksi TBG';
    ...
    {
        return view('Page.Pembatalan.Tabungan.show', compact('tabungan'), ['title' => 'Show Data']);
    }
    ...
    {
        $tabungan->id_transaksi = $request->id_transaksi;
        $tabungan->no_rek = $request->no_rek;
        $tabungan->nama = $request->nama;
        $tabungan->nominal = $request->nominal;
        $tabungan->alasan = $request->alasan;
        $tabungan->user = $request->user;
        $tabungan->keterangan = $request->keterangan;
        $tabungan->created_at = now();
        $tabungan->save();

        // Log Activity
        $LogAksi = '(u) Update Pembatalan Transaksi TBG';
        $this->LogActivity($tabungan, $LogAksi);

        return redirect('pembatalan-tabungan')->with('AlertSuccess', "Pengajuan Pembatalan Transaksi Berhasil Di Edit!");
    }
    ...
    public function ResponApprove(Request $request, $idEncrypt)
    {
        ...
        $data = Tabungan::where('id_tabungan', $ids)->first();
        $jabatan = auth()->user()->jabatan;
        $nama = auth()->user()->nama;
        ...
        switch ($jabatan) {
            ...
            case 'Pembukuan':
                if ($data->status_pembukuan == null) {
                    ...
                    // Log Activity
                    $LogAksi = '(cs) SendedToDirops Pengajuan Pembatalan Transaksi (Tabungan)';
                    $this->LogActivity($data, $LogAksi);
                    // Send Email Single
        ...
        return redirect('pembatalan-tabungan')->with('AlertSuccess', "Pengajuan Berhasil Dilakukan Perubahan Status!");
    }
    ...
    public function ResponReject(Request $request, $idEncrypt)
    {
        ...
        // Log Activity
        $LogAksi = '(cs) Rejected Pengajuan Pembatalan Transaksi (Tabungan)';
        $this->LogActivity($data, $LogAksi);
    ...
    private function statusNothing($data)
    {
        ...
        $status .= '<a class="dropdown-item approve" href="#" data-toggle="modal" data-target="#modalApprove" data-id="' . encrypt($data->id_tabungan) . '" data-kode_form="' . $data->kode_form . '">Approve</a>';
        $status .= '<a class="dropdown-item reject" href="#" data-toggle="modal" data-target="#modalReject" data-id="' . encrypt($data->id_tabungan) . '" data-kode_form="' . $data->kode_form . '">Reject</a>';
        $status .= '</div>';
        $status .= '</div>';
        return $status;
    }
    ...
    private function statusAfter($data, $jabatan, $statusDropdown)
    {
        if (auth()->user()->jabatan == 'Pembukuan') {
            ...
            if ($jabatan == 'Approve') {
                ...
                $status .= '</button>';
                $status .= '<div class="dropdown-menu" aria-labelledby="statusDropdown">';
                $status .= '<a class="dropdown-item approve" href="#" data-toggle="modal" data-target="#modalApprove" data-id="' . encrypt($data->id_tabungan) . '" data-kode_form="' . $data->kode_form . '">SendToDirops</a>';
                $status .= '<a class="dropdown-item reject" href="#" data-toggle="modal" data-target="#modalReject" data-id="' . encrypt($data->id_tabungan) . '" data-kode_form="' . $data->kode_form . '">Reject</a>';
                $status .= '</div>';
                $status .= '</div>';
            }
    ...
    private function SendEmail($data, $userPenerima, $url, $title, $message)
    {
        if (auth()->user()->jabatan == 'Direktur Operasional') {
            Mail::send('email.notif.notif-to-pembukuan',  [
                ...
                $message->subject('Perlu Tindak Lanjut Transaksi (Tabungan)');
            });
        } else {
            Mail::send('email.notif.notif-pengajuan',  [
                ...
                $message->from('tsiksb@bprkusumasumbing.com', 'KSB | Si-PUTa');
                $message->to($userPenerima->email);
                $message->subject('Pengajuan Pembatalan Transaksi (Tabungan)');
            });
        }
