...
use App\Models\Pembatalan\PDeposito;
use App\Models\User;
...
class PDepositoController extends Controller
{
    public function index(Request $request)
    {
            ...
            switch ($jabatan) {
                ...
                case 'Pimpinan Cabang':
                    if (!empty($request->kode)) {
                        $data = PDeposito::where('kode_form', $kode)
                            ->OrderBy('created_at', 'desc')->get();
                    } else {
                        if (!empty($request->min)) {
                            $data = PDeposito::where('id_cabang', $id_cabang)
                ...
                case 'Internal Audit':
                    if (!empty($request->kode)) {
                        ...
                        if (!empty($request->min)) {
                            $data = PDeposito::whereIn('status_pincab', ['Approve', '--'])
                                ->whereBetween('created_at', [$awal, $akhir])
                                ->when($reqCabang != 99, fn($query) => $query->where('id_cabang', $reqCabang))
                                ->get();
                        } elseif (!empty($request->cari)) {
                            $data = PDeposito::where('kode_form', $request->cari)
                                ->orderBy('created_at', 'desc')->get();
                        } else {
                            $data = PDeposito::whereIn('status_pincab', ['Approve', '--'])
                                ->OrderBy('created_at', 'desc')->get();
                ...
                case 'TSI':
                    if (!empty($request->kode)) {
                        ...
                        if (!empty($request->min)) {
                            $data = PDeposito::where('status_dirops', 'Approve')
                                ->whereBetween('created_at', [$awal, $akhir])
                                ->when($reqCabang != 99, fn($query) => $query->where('id_cabang', $reqCabang))
                                ->orderBy('created_at', 'desc')->get();
                        } else {
                            $data = PDeposito::where('status_dirops', 'Approve')->orderBy('created_at', 'desc')->get();
                        }
                    }
                    break;
                default:
        ...
        return view('Page.Pembatalan.Deposito.index', ['title' => 'Pembatalan Transaksi Deposito']);
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($cek == 0) {
            $urut = 0001;
            $nomer = $cabang . '/BTT-DPS/' . $thn . '/0001';
        } else {
            $ambil = PDeposito::all()->last();
        ...
        // Log Activity
        $LogAksi = '(+) Pengajuan Pembatalan Transaksi DPS';
        $this->LogActivity($data, $LogAksi);
        // Send Email
        $userPenerima = User::where('id_cabang', auth()->user()->id_cabang)
            ->where('email', 'not like', '%dummy%')->where('email', 'not like', '%alt%')->where('status', 'Aktif')->where('email', 'like', '%@gmail.com')->first();
    ...
    public function show(PDeposito $pDeposito)
    {
        return view('Page.Pembatalan.Deposito.show', compact('pDeposito'), ['title' => 'Show Data']);
    }



    public function edit(PDeposito $pDeposito)
    {
    ...
    public function update(Request $request, PDeposito $pDeposito)
    {
        $pDeposito->id_transaksi = $request->id_transaksi;
        $pDeposito->no_rek = $request->no_rek;
        $pDeposito->nama = $request->nama;
        $pDeposito->nominal = $request->nominal;
        $pDeposito->alasan = $request->alasan;
        $pDeposito->user = $request->user;
        $pDeposito->keterangan = $request->keterangan;
        $pDeposito->updated_at = now();
        $pDeposito->save();

        // Log Activity
        $LogAksi = '(u) Update Pembatalan Transaksi DPS';
        $this->LogActivity($pDeposito, $LogAksi);

        return redirect('pembatalan-deposito')->with('AlertSuccess', "Pengajuan Pembatalan Transaksi Berhasil Di Edit!");
    }



    public function destroy(PDeposito $pDeposito)
    {
        //
    }
    ...
    public function ResponApprove(Request $request, $idEncrypt)
    {
        ...
        $data = PDeposito::where('id_deposito', $ids)->first();
        $jabatan = auth()->user()->jabatan;
        $nama = auth()->user()->nama;
        ...
        switch ($jabatan) {
            case 'Pimpinan Cabang':
                ...
                // Log Activity
                $LogAksi = '(cs) Approve Pengajuan Pembatalan Transaksi (Deposito)';
                $this->LogActivity($data, $LogAksi);
                // Send Email Double
            ...
            case 'Pembukuan':
                if ($data->status_pembukuan == null) {
                    ...
                    // Log Activity
                    $LogAksi = '(cs) SendedToDirops Pengajuan Pembatalan Transaksi (Deposito)';
                    $this->LogActivity($data, $LogAksi);
                    // Send Email Single
        ...
        return redirect('pembatalan-deposito')->with('AlertSuccess', "Pengajuan Berhasil Dilakukan Perubahan Status!");
    }
    ...
    public function ResponReject(Request $request, $idEncrypt)
    {
        ...
        // Log Activity
        $LogAksi = '(cs) Rejected Pengajuan Pembatalan Transaksi (Deposito)';
        $this->LogActivity($data, $LogAksi);
    ...
    private function statusAfter($data, $jabatan, $statusDropdown)
    {
        if (auth()->user()->jabatan == 'Pembukuan') {
            ...
            if ($jabatan == 'Approve') {
                ...
                $status .= '</button>';
                $status .= '<div class="dropdown-menu" aria-labelledby="statusDropdown">';
                $status .= '<a class="dropdown-item approve" href="#" data-toggle="modal" data-target="#modalApprove" data-id="' . encrypt($data->id_deposito) . '" data-kode_form="' . $data->kode_form . '">SendToDirops</a>';
                $status .= '<a class="dropdown-item reject" href="#" data-toggle="modal" data-target="#modalReject" data-id="' . encrypt($data->id_deposito) . '" data-kode_form="' . $data->kode_form . '">Reject</a>';
                $status .= '</div>';
                $status .= '</div>';
            }
