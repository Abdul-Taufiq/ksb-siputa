...
class InventarisController extends Controller
{
    ...
    private $subjek = 'Pengajuan Pembatalan Transaksi Inventaris';
    private $subjek_status = 'Status | Pengajuan Pembatalan Transaksi Inventaris';
    ...
    public function index(Request $request)
    {
        ...
        if (request()->ajax()) {
            ...
            return DataTables()->of($data)
                ...
                ->addColumn('action', function ($data) {
                    $button = '<a data-toggle="modal" data-target="#myModal' . $data->id_inventaris . '" id="' . $data->id_inventaris . '"
                    ...
                    switch (auth()->user()->jabatan) {
                        ...
                        case 'Kepala Kantor Kas':
                            if ($data->status_pincab == "Approve" || $data->status_pincab == "Reject") {
                                ...
                                $button .= '<a data-toggle="modal" data-target="#modalEdit' . $data->id_inventaris . '" id="' . $data->id_inventaris . '"
        ...
        return view('Page.Pembatalan.Inventaris.index', ['title' => 'Pembatalan Transaksi Inventaris']);
    }
    ...
    public function store(Request $request)
    {
        ...
        // Log Activity
        $LogAksi = '(+) Pengajuan Pembatalan Transaksi INV';
    ...
    {
        return view('Page.Pembatalan.Inventaris.show', compact('inventaris'), ['title' => ' Show Data']);
    }
    ...
    {
        $inventaris->id_transaksi = $request->id_transaksi;
        $inventaris->nomor_seri = $request->nomor_seri;
        $inventaris->nama_barang = $request->nama_barang;
        $inventaris->nominal = $request->nominal;
        $inventaris->alasan = $request->alasan;
        $inventaris->user = $request->user;
        $inventaris->keterangan = $request->keterangan;
        $inventaris->updated_at = now();
        $inventaris->save();

        // Log Activity
        $LogAksi = '(u) Update Pembatalan Transaksi INV';
        $this->LogActivity($inventaris, $LogAksi);

        return redirect('pembatalan-inventaris')->with('AlertSuccess', "Pengajuan Pembatalan Transaksi Berhasil Di Edit!");
    }
    ...
    public function ResponApprove(Request $request, $idEncrypt)
    {
        ...
        $data = Inventaris::where('id_inventaris', $ids)->first();
        $jabatan = auth()->user()->jabatan;
        $nama = auth()->user()->nama;
        ...
        switch ($jabatan) {
            case 'Pimpinan Cabang':
                ...
                // Log Activity
                $LogAksi = '(cs) Approve Pengajuan Pembatalan Transaksi (Inventaris)';
                $this->LogActivity($data, $LogAksi);
                // Send Email Double
            ...
            case 'Pembukuan':
                if ($data->status_pembukuan == null) {
                    ...
                    // Log Activity
                    $LogAksi = '(cs) SendedToDirops Pengajuan Pembatalan Transaksi (Inventaris)';
                    $this->LogActivity($data, $LogAksi);
                    // Send Email Single
        ...
        return redirect('pembatalan-inventaris')->with('AlertSuccess', "Pengajuan Berhasil Dilakukan Perubahan Status!");
    }
    ...
    public function ResponReject(Request $request, $idEncrypt)
    {
        ...
        // Log Activity
        $LogAksi = '(cs) Rejected Pengajuan Pembatalan Transaksi (Inventaris)';
        $this->LogActivity($data, $LogAksi);
    ...
    private function statusNothing($data)
    {
        ...
        $status .= '<a class="dropdown-item approve" href="#" data-toggle="modal" data-target="#modalApprove" data-id="' . encrypt($data->id_inventaris) . '" data-kode_form="' . $data->kode_form . '">Approve</a>';
        $status .= '<a class="dropdown-item reject" href="#" data-toggle="modal" data-target="#modalReject" data-id="' . encrypt($data->id_inventaris) . '" data-kode_form="' . $data->kode_form . '">Reject</a>';
        $status .= '</div>';
        $status .= '</div>';
        return $status;
    }
    ...
    private function statusAfter($data, $jabatan, $statusDropdown)
    {
        if (auth()->user()->jabatan == 'Pembukuan') {
            ...
            if ($jabatan == 'Approve') {
                ...
                $status .= '</button>';
                $status .= '<div class="dropdown-menu" aria-labelledby="statusDropdown">';
                $status .= '<a class="dropdown-item approve" href="#" data-toggle="modal" data-target="#modalApprove" data-id="' . encrypt($data->id_inventaris) . '" data-kode_form="' . $data->kode_form . '">SendToDirops</a>';
                $status .= '<a class="dropdown-item reject" href="#" data-toggle="modal" data-target="#modalReject" data-id="' . encrypt($data->id_inventaris) . '" data-kode_form="' . $data->kode_form . '">Reject</a>';
                $status .= '</div>';
                $status .= '</div>';
            }
