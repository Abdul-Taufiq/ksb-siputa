...
use App\Models\Perubahan\Deposito;
use App\Models\User;
...
class DepositoController extends Controller
{
    ...
    private $subjek = 'Pengajuan Perubahan Data (Deposito)';
    private $subjek_status = 'Status | Pengajuan Perubahan Data (Deposito)';
    ...
    public function index(Request $request)
    {
        ...
        if (request()->ajax()) {
            switch ($jabatan) {
                ...
                case 'Pimpinan Cabang':
                    if (!empty($request->kode)) {
                        ...
                        if (!empty($request->min)) {
                            $data = Deposito::where('id_cabang', $id_cabang)
                ...
                case 'Internal Audit':
                    if (!empty($request->kode)) {
                        ...
                        if (!empty($request->min)) {
                            $data = Deposito::whereIn('status_pincab', ['Approve', '--'])
                                ->whereBetween('created_at', [$awal, $akhir])
                                ->when($reqCabang != 99, fn($query) => $query->where('id_cabang', $reqCabang))
                                ->get();
                        } elseif (!empty($request->cari)) {
                            $data = Deposito::where('kode_form', $request->cari)
                                ->orderBy('created_at', 'desc')->get();
                        } else {
                            $data = Deposito::whereIn('status_pincab', ['Approve', '--'])
                                ->OrderBy('created_at', 'desc')->get();
                ...
                case 'TSI':
                    if (!empty($request->kode)) {
                        ...
                        if (!empty($request->min)) {
                            $data = Deposito::where('status_dirops', 'Approve')
                                ->whereBetween('created_at', [$awal, $akhir])
                                ->when($reqCabang != 99, fn($query) => $query->where('id_cabang', $reqCabang))
                                ->orderBy('created_at', 'desc')->get();
                        } else {
                            $data = Deposito::where('status_dirops', 'Approve')->orderBy('created_at', 'desc')->get();
                        }
                    }
                    break;
                default:
            ...
            return DataTables()->of($data)
                ...
                ->addColumn('action', function ($data) {
                    $button = '<a data-toggle="modal" data-target="#myModal' . $data->id_deposito . '" id="' . $data->id_deposito . '"
                    ...
                    switch (auth()->user()->jabatan) {
                        ...
                        case 'Kepala Kantor Kas':
                            if ($data->status_pincab == "Approve" || $data->status_pincab == "Reject") {
                                ...
                                $button .= '<a data-toggle="modal" data-target="#modalEdit' . $data->id_deposito . '" id="' . $data->id_deposito . '"
        ...
        return view('Page.Perubahan.MasterDeposito.index', ['title' => 'Perubahan Transaksi Master Deposito']);
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($cek == 0) {
            $urut = 0001;
            $nomer = $cabang . '/PRT-DPS/' . $thn . '/0001';
        } else {
            $ambil = Deposito::all()->last();
        ...
        if ($request->jns_deposito == 'Lainnya') {
            $data->jns_deposito = $request->keterangan_jns_deposito;
            $data->data_salah = $request->data_salah;
            $data->pembetulan = $request->pembetulan;
        } else if ($request->jns_deposito == 'Valuta dan Suku Bunga') {
            $data->jns_deposito = $request->jns_deposito;
            $data->data_salah = 'Nominal: ' . $request->nominal_s . ' - ' . 'Suku Bunga: ' . $request->suku_bunga_s;
            $data->pembetulan = 'Nominal: ' . $request->nominal_p . ' - ' . 'Suku Bunga: ' . $request->suku_bunga_p;
        } else {
            $data->jns_deposito = $request->jns_deposito;
            $data->data_salah = $request->data_salah;
            $data->pembetulan = $request->pembetulan;
        ...
        // Log Activity
        $LogAksi = '(+) Pengajuan Perubahan Transaksi DPS';
        ...
        return redirect('perubahan-deposito')->with('AlertSuccess', "Pengajuan Perubahan Transaksi Berhasil Dikirim!");
    }


    public function show(Deposito $deposito)
    {
        return view('Page.Perubahan.MasterDeposito.show', compact('deposito'), ['title' => 'Show Data']);
    }
    ...
    {
        if ($request->jns_deposito == 'Lainnya') {
            $deposito->jns_deposito = $request->keterangan_jns_deposito;
            $deposito->data_salah = $request->data_salah;
            $deposito->pembetulan = $request->pembetulan;
        } else if ($request->jns_deposito == 'Valuta dan Suku Bunga') {
            $deposito->jns_deposito = $request->jns_deposito;
            $deposito->data_salah = 'Nominal: ' . $request->nominal_s . ' - ' . 'Suku Bunga: ' . $request->suku_bunga_s;
            $deposito->pembetulan = 'Nominal: ' . $request->nominal_p . ' - ' . 'Suku Bunga: ' . $request->suku_bunga_p;
        } else {
            $deposito->jns_deposito = $request->jns_deposito;
            $deposito->data_salah = $request->data_salah;
            $deposito->pembetulan = $request->pembetulan;
        ...
        $deposito->no_rek = $request->no_rek;
        $deposito->nama_nasabah = $request->nama;
        $deposito->user = $request->user;

        $deposito->alasan = $request->alasan;
        ...
        // Log Activity
        $LogAksi = '(u) Update Perubahan Transaksi DPS';
        $this->LogActivity($deposito, $LogAksi);

        return redirect('perubahan-deposito')->with('AlertSuccess', "Pengajuan Perubahan Transaksi Berhasil Di Edit!");
    }
    ...
    public function ResponApprove(Request $request, $idEncrypt)
    {
        ...
        $data = Deposito::where('id_deposito', $ids)->first();
        $jabatan = auth()->user()->jabatan;
        $nama = auth()->user()->nama;
        ...
        // Log Activity
        $LogAksi = '(cs) Approve Pengajuan Perubahan Transaksi (Deposito)';
        $this->LogActivity($data, $LogAksi);

        return redirect('perubahan-deposito')->with('AlertSuccess', "Pengajuan Berhasil Dilakukan Perubahan Status!");
    }
    ...
    public function ResponReject(Request $request, $idEncrypt)
    {
        ...
        // Log Activity
        $LogAksi = '(cs) Rejected Pengajuan Perubahan Transaksi (Deposito)';
        $this->LogActivity($data, $LogAksi);
    ...
    private function statusNothing($data)
    {
        ...
        $status .= '<a class="dropdown-item approve" href="#" data-toggle="modal" data-target="#modalApprove" data-id="' . encrypt($data->id_deposito) . '" data-kode_form="' . $data->kode_form . '">Approve</a>';
        $status .= '<a class="dropdown-item reject" href="#" data-toggle="modal" data-target="#modalReject" data-id="' . encrypt($data->id_deposito) . '" data-kode_form="' . $data->kode_form . '">Reject</a>';
        $status .= '</div>';
        $status .= '</div>';
        return $status;
    }
